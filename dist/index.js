...
const localhost=host=>process.getuid()===0&&(host==='localhost'||host==='0.0.0.0'||host==='127.0.0.1');
const urlArgs=(referer,def)=>Object.assign(def,url__WEBPACK_IMPORTED_MODULE_0__["parse"](referer,true).query);const getRemoteAddress=remoteAddress=>remoteAddress.split(':')[3]===undefined?'localhost':remoteAddress.split(':')[3];/* harmony default export */ __webpack_exports__["default"] = (({request:{headers:{referer}},client:{conn:{remoteAddress}}},{user,host,port,auth,pass,key},command)=>({args:localhost(host)?loginOptions(command,remoteAddress):sshOptions(urlArgs(referer,{host:address(referer,user,host),port:`${port}`,pass,command,auth}),key),user:localhost(host)||user!==''||user.includes('@')||address(referer,user,host).includes('@')}));
function parseCommand(command,path){if(command==='login'&&path===undefined)return'';return path!==undefined?`$SHELL -c "cd ${path};${command==='/var/cld/bin/cldx'?'$SHELL':command}"`:command;}
function sshOptions({pass,path,command,host,port,auth},key){const cmd=parseCommand(command,path);const sshRemoteOptsBase=['ssh',host,'-t','-p',port,'-o',`PreferredAuthentications=${auth}`];if(key){return sshRemoteOptsBase.concat(['-i',key,cmd]);}if(pass){return['sshpass','-p',pass].concat(sshRemoteOptsBase,[cmd]);}if(cmd===''){return sshRemoteOptsBase;}return sshRemoteOptsBase.concat([cmd]);}
function loginOptions(command,remoteAddress){return command==='login'?[command,'-h',getRemoteAddress(remoteAddress)]:[command];}
...
function loginOptions(command,remoteAddress){return command==='/var/cld/bin/cldx'?[command,cldid]:[command];}
...
const distDir=path__WEBPACK_IMPORTED_MODULE_7__["join"](__dirname,'client');const trim=str=>str.replace(/\/*$/,'');function createServer({base,port,host,title},{key,cert}){const basePath=trim(base);_emitter__WEBPACK_IMPORTED_MODULE_11__["default"].emit('debug',`key: ${key}, cert: ${cert}, port: ${port}, base: ${base}, title: ${title}`);const html=(req,res)=>res.send(`<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
...
function asyncGeneratorStep(gen,resolve,reject,_next,_throw,key,arg){try{var info=gen[key](arg);var value=info.value;}catch(error){reject(error);return;}if(info.done){resolve(value);}else{Promise.resolve(value).then(_next,_throw);}}function _asyncToGenerator(fn){return function(){var self=this,args=arguments;return new Promise(function(resolve,reject){var gen=fn.apply(self,args);function _next(value){asyncGeneratorStep(gen,resolve,reject,_next,_throw,"next",value);}function _throw(err){asyncGeneratorStep(gen,resolve,reject,_next,_throw,"throw",err);}_next(undefined);});};}function loadSSL(_x){return _loadSSL.apply(this,arguments);}function _loadSSL(){_loadSSL=_asyncToGenerator(function*(ssl){if(lodash_isUndefined__WEBPACK_IMPORTED_MODULE_0___default()(ssl.key)||lodash_isUndefined__WEBPACK_IMPORTED_MODULE_0___default()(ssl.cert))return{};const files=[Object(fs_extra__WEBPACK_IMPORTED_MODULE_1__["readFile"])(Object(path__WEBPACK_IMPORTED_MODULE_2__["resolve"])(ssl.key)),Object(fs_extra__WEBPACK_IMPORTED_MODULE_1__["readFile"])(Object(path__WEBPACK_IMPORTED_MODULE_2__["resolve"])(ssl.cert))];const[key,cert]=yield Promise.all(files);return{key,cert};});return _loadSSL.apply(this,arguments);}
